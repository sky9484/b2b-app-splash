...
function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-8">
          {[["How it works", "#how-it-works"], ["Pricing", "#pricing"], ["FAQ", "#faq"]].map(([label, href]) => (
            <a key={label} href={href} className="text-sm font-medium transition-opacity hover:opacity-60" style={{ color: "var(--ink)" }}>{label}</a>
          ))}
        </div>
      ...
      {mobileOpen && (
        <div className="md:hidden px-4 pb-4 pt-2 flex flex-col gap-3" style={{ borderTop: "1px solid var(--outline)", backgroundColor: "#fff" }}>
          {[["How it works", "#how-it-works"], ["Pricing", "#pricing"], ["FAQ", "#faq"]].map(([label, href]) => (
            <a key={label} href={href} className="text-sm font-medium py-1" style={{ color: "var(--ink)" }} onClick={() => setMobileOpen(false)}>{label}</a>
          ))}
          <div className="flex flex-col gap-2 pt-2" style={{ borderTop: "1px solid var(--outline)" }}>
            <Link to="/login" className="text-sm font-medium text-center py-2 rounded-lg" style={{ color: "var(--indigo)", border: "1px solid var(--outline)" }}>Sign in</Link>
            <Link to="/login" className="text-sm font-semibold text-white text-center py-2 rounded-lg" style={{ backgroundColor: "var(--indigo)" }}>Get Started</Link>
          </div>
        </div>
      )}
    </nav>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-20 md:py-28 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link to="/login" className="inline-flex items-center justify-center gap-2 rounded-lg px-6 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "var(--success)" }}>
        ...
        <div className="flex justify-center md:justify-end">
          <div className="w-full max-w-sm rounded-2xl p-6 shadow-2xl" style={{ backgroundColor: "#fff", border: "1px solid var(--outline)" }}>
            ...
            <div className="rounded-xl p-4 mb-4" style={{ backgroundColor: "var(--success-bg)", border: "1px solid var(--success-border)" }}>
              ...
              <div className="flex items-center justify-between gap-2">
                <span className="text-2xl font-semibold tabular-nums" style={{ color: "var(--success)" }}>
            ...
            <div className="space-y-2 text-sm mb-4">
              <div className="flex justify-between">
                ...
                <span className="font-medium tabular-nums" style={{ color: "var(--ink)" }}>RM {FEE.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span style={{ color: "var(--ink-3)" }}>Estimated arrival</span>
...
function TrustBar() {
  ...
  return (
    <section className="py-10" style={{ backgroundColor: "#fff", borderBottom: "1px solid var(--outline)" }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.label} className="flex flex-col items-center text-center px-4 py-2">
              <span className="text-3xl font-semibold tabular-nums tracking-tight" style={{ color: "var(--indigo)" }}>{s.value}</span>
              <span className="text-sm mt-1" style={{ color: "var(--ink-3)" }}>{s.label}</span>
            </div>
          ))}
        </div>
      </div>
...
function HowItWorks() {
  const steps = [
    { number: "01", title: "Add a recipient", description: "Enter your Philippine recipient's name, bank, and account number. Save them for future transfers.", color: "var(--indigo)" },
    { number: "02", title: "Enter amount & review rate", description: "Type the MYR amount. See the live PHP rate, exact fee, and what your recipient gets — before you confirm.", color: "var(--success)" },
    { number: "03", title: "Done — money arrives", description: "Pay via FPX. Splash settles to your recipient's account in under 5 minutes. A PDF receipt is emailed instantly.", color: "var(--warning)" },
  ];
  return (
    <section id="how-it-works" className="py-20" style={{ backgroundColor: "var(--page)" }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight mb-3" style={{ color: "var(--indigo)" }}>How it works</h2>
          <p className="text-base max-w-xl mx-auto" style={{ color: "var(--ink-3)" }}>Three steps from login to your recipient's account. No paperwork, no branch visits.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {steps.map((step) => (
            <div key={step.number} className="rounded-2xl p-7 bg-white" style={{ border: "1px solid var(--outline)" }}>
              ...
              <h3 className="text-lg font-semibold mb-2" style={{ color: "var(--indigo)" }}>{step.title}</h3>
              <p className="text-sm leading-relaxed" style={{ color: "var(--ink-3)" }}>{step.description}</p>
            </div>
          ))}
        </div>
        <div className="mt-10 text-center">
          <Link to="/login" className="inline-flex items-center gap-2 rounded-lg px-6 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "var(--indigo)" }}>
            Get Started <ArrowRight size={16} />
          </Link>
        </div>
      </div>
    ...
    <section id="pricing" className="py-20" style={{ backgroundColor: "#fff" }}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight mb-3" style={{ color: "var(--indigo)" }}>Simple, honest pricing</h2>
          <p className="text-base max-w-xl mx-auto" style={{ color: "var(--ink-3)" }}>One flat fee, no surprises. See how we compare.</p>
        </div>
        <div className="overflow-x-auto rounded-2xl" style={{ border: "1px solid var(--outline)" }}>
          <table className="w-full min-w-[560px] border-collapse">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--outline)" }}>
                <th className="px-4 py-4 text-left text-sm font-medium" style={{ color: "var(--ink-3)", width: "28%" }}>Feature</th>
                <th className="px-4 py-4 text-center text-sm font-semibold" style={{ color: "#fff", backgroundColor: "var(--indigo)", width: "24%" }}>
                  <div className="flex flex-col items-center gap-1">
                    <span>Splash</span>
                    <span className="text-xs font-medium rounded-full px-2 py-0.5" style={{ backgroundColor: "var(--indigo-hover)", color: "#fff" }}>Recommended</span>
                  </div>
                </th>
                <th className="px-4 py-4 text-center text-sm font-medium" style={{ color: "var(--ink-3)", width: "24%" }}>Traditional Banks</th>
                <th className="px-4 py-4 text-center text-sm font-medium" style={{ color: "var(--ink-3)", width: "24%" }}>Other Services</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row, i) => (
                <tr key={row.feature} style={{ borderBottom: i < rows.length - 1 ? "1px solid var(--outline)" : "none" }}>
                  <td className="px-4 py-4 text-sm font-medium" style={{ color: "var(--ink)" }}>{row.feature}</td>
                  <td className="px-4 py-4 text-sm text-center font-semibold" style={{ backgroundColor: "rgba(67,56,202,0.04)", color: "var(--indigo)" }}>
                    {typeof row.splash === "boolean" ? <BoolCell val={row.splash} /> : row.splash}
                  </td>
                  <td className="px-4 py-4 text-sm text-center" style={{ color: "var(--ink-3)" }}>
        ...
        <p className="text-xs text-center mt-4" style={{ color: "var(--ink-3)" }}>Comparison based on publicly available information. Rates may vary.</p>
      </div>
    </section>
  );
}
...
function Features() {
  ...
  return (
    <section className="py-20" style={{ backgroundColor: "var(--page)" }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight mb-3" style={{ color: "var(--indigo)" }}>Built for Malaysian businesses</h2>
          <p className="text-base max-w-xl mx-auto" style={{ color: "var(--ink-3)" }}>Everything you need to manage cross-border payouts — without the complexity.</p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {features.map((f) => (
            <div key={f.title} className="rounded-2xl p-6 bg-white flex flex-col gap-4" style={{ border: "1px solid var(--outline)" }}>
              ...
              <div>
                <h3 className="text-base font-semibold mb-1.5" style={{ color: "var(--indigo)" }}>{f.title}</h3>
                <p className="text-sm leading-relaxed" style={{ color: "var(--ink-3)" }}>{f.description}</p>
              </div>
            </div>
          ))}
        </div>
...
function FAQItem({ item }: { item: { q: string; a: string } }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--outline)", backgroundColor: "#fff" }}>
      ...
      {open && (
        <div className="px-5 pb-4 text-sm leading-relaxed" style={{ color: "var(--ink-3)", borderTop: "1px solid var(--outline)" }}>
  ...
  return (
    <section id="faq" className="py-20" style={{ backgroundColor: "#fff" }}>
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-semibold tracking-tight mb-3" style={{ color: "var(--indigo)" }}>Frequently asked questions</h2>
          <p className="text-base" style={{ color: "var(--ink-3)" }}>Everything you need to know before your first transfer.</p>
        </div>
        <div className="flex flex-col gap-3">
          {FAQ_ITEMS.map((item) => <FAQItem key={item.q} item={item} />)}
        </div>
        <div className="mt-10 text-center">
          <p className="text-sm mb-4" style={{ color: "var(--ink-3)" }}>Still have questions?</p>
          <a href="mailto:support@splashpay.io" className="inline-flex items-center gap-2 text-sm font-medium transition-opacity hover:opacity-70" style={{ color: "var(--indigo)" }}>
  ...
  return (
    <section className="py-20 relative overflow-hidden" style={{ backgroundColor: "var(--sidebar-dark)" }}>
      ...
      <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
        ...
        <Link to="/login" className="inline-flex items-center gap-2 rounded-lg px-8 py-3.5 text-sm font-semibold text-white transition-opacity hover:opacity-90" style={{ backgroundColor: "var(--success)" }}>
          Get Started <ArrowRight size={16} />
        </Link>
